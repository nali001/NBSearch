...
def preprocessing(path, out_path):
    """
    ...
    Typical Usage:
    -------------
    preprocessing('testdata/notebooks/')
    """
    # --------------------------- Method definitions ----------------------------
    ...
    # Extract the names of notebooks
    ...
    # Tokenize codes with no comments 
    ...
    # Keep the cells that both have functions and comments??? 
    df_nb_non_empty_function_body = df_nb[df_nb['function_token'].map(len) > 0]
    
    # Split the codes into those with comments and without comments 
    ...
    # Transform the data type into lists 
    ...
    # Save preprocessors
    with open(os.getcwd() + '/' + out_path + '/cell_pp.dpkl', 'wb') as f:
        dpickle.dump(func_pp, f)
    with open(os.getcwd() + '/' + out_path + '/comments_pp.dpkl', 'wb') as f:
        dpickle.dump(comments_pp, f)

    # Save the processed data
    np.save(os.getcwd() + '/' + out_path + '/train_comments_vecs.npy', train_comments_vecs)
    np.save(os.getcwd() + '/' + out_path + '/train_cell_vecs.npy', train_func_vecs)
    print("Finish processing data")

def preprocess_language_model_data(out_path):
    """
    Preprocess notebooks data for doc2vec model.
    """
    predicted = pd.read_csv(os.getcwd() + '/' + out_path + '/train_rows.csv')
    f= open(os.getcwd() + '/' + out_path + '/corpus.txt',"w+")
